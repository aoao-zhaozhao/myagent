...
```
已具备: LangGraph 引擎 + 14 个工具 + RAG 知识库 + JS/API/SPA 分析 + LFI 专项验证 + Agent 工作台
...
## v0.8 — Agent UI 2.0 打磨 + 协议增强

> v0.7 已经合并核心工作台能力；v0.8 继续做协议、可靠性和交互细节打磨

### 为什么放在这里？

v0.5-v0.7 给了 Agent 更多工具，v0.7 已经把工具轨迹和阶段进度做进前端。后续 v0.8 不再作为一次大重写，而是把结果协议做得更结构化，并补齐 CPU 密集型同步工具的更强中断能力。
...
v0.8 继续用: vanilla JS + marked.js + Prism.js + CSS variables
保持: 单文件 HTML 或拆分为 web/ 目录下的 index.html + style.css + app.js
后端改动: 协议字段和同步工具超时
前端改动: 在 v0.7 工作台基础上拆分和增强
```
...
v0.7  ★ 当前  LFI 专项 + 可观测工作台 + 停止扫描 ~3300 行
v0.8  UI 打磨   结构化协议 + 同步工具中断增强     ~3800 行
